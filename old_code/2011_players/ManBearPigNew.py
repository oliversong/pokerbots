...
class ManBearPigBotNew(TheBostonDerby):
    def __init__(self):
        TheBostonDerby.__init__(self)
        self.name = "ManBearPigBotNew"
        self.bot = manbearpigbotnew()

class manbearpigbotnew(LukeBotNew):
    def __init__(self):
...
###########Strategy to Take On Flop###################
    def flop(self, brain):
        ...
        if self.eq == 0: ##0
            ...
            self.fir = self.randaction(Bet(randint(15,25)) , Bet(randint(20,30)), 50)
            self.che = Bet(randint(15,25)) 
            self.chebet = [[Raise(self.min), self.valuebet], [Call(), brain.pot], [Fold()]]
            self.dealbet = [[Call(), min(50, brain.stack/10)], [Call(), brain.pot], [Fold()]]          
            self.betrai = [[self.randaction(Call(), Fold(), 50), min(100, brain.stack/3)], [Fold(), self.oppallin], [Fold()]] 
            self.chebetrai = [[Fold(), self.myallin/3], [Fold, self.oppallin], [Fold()]] 
            self.betrairai = [[Call(), self.myallin/3], [Fold(), self.oppallin], [Fold()]]
            if brain.opp_stats['AF']<1:
                self.betrai= [[Call(), 60], [self.randaction(Call(), Fold(), 40), 100], [Fold()]]
            if brain.opp_stats['CHBR']> .2:
                self.chebetrai = [[Call(), 50], [Call(), 100], [Fold()]]
        elif self.eq == 4: ##91
...
#############Action to Take on Turn###################
    def turn(self, brain):
        ...
        if self.eq == 0: ##0
            ...
            self.chebetrai = [[Fold(), self.myallin/3], [Fold, self.oppallin], [Fold()]] 
            self.betrairai = [[Call(), self.myallin/3], [Fold(), self.oppallin], [Fold()]]
            if brain.opp_stats['AF']< .8:
                self.betrai= [[Call(), 60], [self.randaction(Call(), Fold(), 40), 80], [Fold()]]
            if brain.opp_stats['CHBR']> .2:
                self.chebetrai = [[Call(), 50], [Call(), 100], [Fold()]]
        elif self.eq == 4: ##85
            self.fir = Bet(randint(20,30)) 
    ...
    def river(self, brain):
        ...
        if self.eq == 0: ##0
            ...
            self.fir = self.randaction(Bet(randint(25,50)) , Bet(randint(50,75)), 50)
            self.che = Bet(randint(20, 75)) 
            self.chebet = [[Raise(self.min), self.valuebet], [Call(), brain.pot], [Fold()]]
            self.dealbet = [[Call(), min(50, brain.stack/10)], [Call(), brain.pot], [Fold()]]          
            self.betrai = [[Call(), min(150, brain.stack/2)], [Fold(), self.oppallin], [Fold()]] 
            self.chebetrai = [[Fold(), self.myallin/3], [Fold, self.oppallin], [Fold()]] 
            self.betrairai = [[Call(), self.myallin/3], [Fold(), self.oppallin], [Fold()]]
            if brain.opp_stats['AF']< .7:
                self.betrai= [[Call(), 60], [self.randaction(Call(), Fold(), 40), 80], [Fold()]]
