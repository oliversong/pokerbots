...
class TheBostonDerbyC:
    def __init__(self):
        """This is a very simple player that demonstrates the API and is a good
        template for getting started
        """
        # my name
        self.name = "TheBostonDerbyC"
        self.stack = None
        self.hands_played = None
        self.last_hand = -1
        self.loose = TheBostonDerbyLoose()
        self.tight = TheBostonDerbyTight()
        self.bot = self.loose
        self.hand = None
        self.stack = None
        self.pip = None
    ...
    def update(self):
        self.bot.hand = self.hand
        self.bot.stack = self.stack
        self.bot.pip = self.pip
        self.bot.button = self.button
        self.bot.opponent = self.opponent
        self.bot.bb = self.bb
        self.bot.sb = self.sb
        self.bot.hands_played = self.hands_played
        self.bot.board = self.board
        self.bot.legal = self.legal
        self.bot.actions = self.actions
        self.bot.last = self.last
        self.bot.pot = self.pot
...
class TheBostonDerbyLoose:
    def __init__(self):
        """This is a very simple player that demonstrates the API and is a good
        template for getting started
        """
        # my name
        self.name = "TheBostonDerbyLoose"
...
class TheBostonDerbyTight:
    def __init__(self):
        """This is a very simple player that demonstrates the API and is a good
        template for getting started
        """
        # my name is tight!
        self.name = "TheBostonDerbyTight"
        ...
        if not self.board.flop():
            if stack_size < 30*self.bb:
                ...
                if self.ev > .9 or (self.stats['pfhb'] > 0.25 and self.hands_played > 12):
                    ...
                    self.bound = int(HIGHb * stack_size)
                elif self.ev > .5:
                    self.bound = 5 * self.bb
                else:
                    self.bound = self.bb
    ...
    def preflop(self, brain):
        ...
        if self.pfeq == 0:
            ...
            if self.first:
                ...
                my_action = Raise(randint(int(LOWb*stack_size),int(MIDb*stack_size))) #low-mid
            elif last_action == RAISE:
                if self.my_last_action['action'] == CHECK or last_act[1].amount/float(stack_size) > LOWb: #TODO: this should be pot odds!
                    my_action = Fold()
                else:
                    my_action = Call()
        elif self.pfeq == 2:
            if self.first:
                my_action = Raise(LOWb*stack_size)
            elif last_action == CALL:
                my_action = Raise(randint(int(LOWb*stack_size),int(MIDb*stack_size))) #low to mid
            elif last_action == RAISE:
                if self.my_last_action['action'] == CHECK or last_act[1].amount/float(stack_size) > MIDb:
                    my_action = Fold()
                else:
                    my_action = Call()
        elif self.pfeq == 3:
            if self.first:
                ...
                my_action = Raise(MIDb*stack_size)
            elif last_action == RAISE:
                if self.my_last_action['action'] == CHECK or last_act[1].amount/float(stack_size) > MIDb:
                    my_action = Fold()
                else:
        ...
        if self.pfeq == 0:
            if eq == 0:
                ...
                if self.first:
                    ...
                    if last_act[1].amount/float(stack_size) <= LOWb:
                        my_action = Call()
                    else:
                        my_action = Fold()
            elif eq == 2:
                if brain.stats['af'] < .5:
                    ...
                    if self.my_last_action['first']:
                        if last_act[1].amount/float(stack_size) <= LOWb:
                            my_act = Call()
                        elif last_act[1].amount/float(stack_size) <= HIGHb:
                            if brain.stats['pfr'] < .2 : #or brain.stats['cr'] < .4
                                my_action = Fold()
                            else:
                                my_action = Call()
                ...
                if self.first or last_action == CHECK:
                    my_action = Bet(randint(int(LOWb*stack_size),int(MIDb*stack_size))) #low-mid
                elif last_action == BET:
                        if last_act[1].amount/float(stack_size) <= MIDb:
                            my_action = Call()
                        else:
                            my_action = Fold()
                elif last_action == RAISE:
                ...
                if self.first:
                    my_action = Bet(randint(int(MIDb*stack_size),int(HIGHb*stack_size))) #mid_high
                elif last_action == CHECK:
                    my_action = Bet(int(LOWb*stack_size))
                elif last_action == BET:
                    my_action = Raise(int(MIDb*stack_size))
                elif last_action == RAISE:
                    if self.my_last_action['first']:
                        if last_act[1].amount/float(stack_size) <= MIDb:
                            my_action = Call()
            ...
            if eq == 0:
                ...
                if self.first:
                    if randint(0,99) < 50:
                        ...
                        my_action = Bet(randint(self.min_bet,int(LOWb*stack_size)))
                elif last_action == CHECK:
                    my_action = Check()
                elif last_action in [BET, RAISE]:
                    if last_act[1].amount/float(stack_size) <= LOWb:
                ...
                if brain.stats['af'] < .5:
                    ...
                    my_action = Bet(randint(int(LOWb*stack_size),int(MIDb*stack_size)))
                elif last_action == CHECK:
                    if randint(0,99) < 50:
                        my_action = Check()
                    else:
                ...
                if self.first or last_action == CHECK:
                    my_action = Bet(int(MIDb*stack_size))
                elif last_action == BET:
                        if last_act[1].amount/float(stack_size) <= LOWb:
                            my_action = Raise(int(MIDb*stack_size))
                        elif last_act[1].amount/float(stack_size) <= HIGHb:
            ...
            if eq == 0:
                ...
                if self.first or last_action == Check:
                    ...
                    if self.my_last_action['first']:
                        if last_act[1].amount/float(stack_size) <= HIGHb:
                            my_action = Call()
                        else:
                            my_action = Fold()
                    elif self.my_last_action['action'] == BET:
                ...
                if self.first:
                    my_action = Bet(randint(int(MIDb*stack_size),int(HIGHb*stack_size)))
                elif last_action == CHECK:
                    my_action = Bet(int(MIDb*stack_size))
                elif last_action == BET:
                    my_action = Raise(int(MIDb*stack_size))
        ...
        if self.pfeq == 0:
            if eq == 0:
                ...
                if brain.stats['af'] < .5:
                    ...
                    if last_act[1].amount/float(stack_size) <= LOWb:
                        my_action = Call()
                    elif last_act[1].amount/float(stack_size) <= MIDb:
                        if brain.stats['af'] > 1.5:
                            my_action = Call()
                        else:
                            my_action = Fold()
            ...
            if eq == 0:
                ...
                if self.first:
                    ...
                    if last_act[1].amount/float(stack_size) <= MIDb:
                        my_action = Raise(HIGHb*stack_size)
                    elif last_act[1].amount/float(stack_size) <= HIGHb:
                        my_action = Call()
                    else:
                        my_action = Fold()
                ...
                if self.first:
                    if brain.stats['af'] < .5:
                        ...
                        if randint(0,99) < 50:
                            ...
                            my_action = Bet(randint(int(self.min_bet),int(LOWb*stack_size)))
                elif last_action == CHECK:
                    my_action = Bet(randint(int(LOWb*stack_size),int(MIDb*stack_size)))
                elif last_action == BET:
                    if last_act[1].amount/float(stack_size) <= LOWb:
        ...
        if eq == 2:
            ...
            if self.first:
                if stack_size < 200 or stack_size > 600:
                    ...
                    if randint(0,99) < 50:
                        my_action = Bet(HIGHb*stack_size)
                    else:
                        my_action = Bet(MIDb*stack_size)
            elif last_action == CHECK:
                my_action = Bet(MIDb*stack_size)
